...
## Adapted for realsense by: Ashesh
## Date: September, 30, 2020
# Purpose: realsense to use Yolact using Pytorch
...
from data.config import MEANS, COLORS
...
from data.config import cfg, set_cfg, set_dataset
...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/yolact_base_31999_800000.pth', type=str,
...
class detect:
    ...
    def evalvideo(self, net:Yolact, path:str):
        ...
        def transform_frame(frames):
            frames = [np.asarray(frame, np.float32) for frame in frames]

            with torch.no_grad():
                # print("type", type(frames[0]))
                frames = [torch.from_numpy(frame).cuda().float() for frame in frames]
                print("transformShape", frames[0].shape)
                transformFrame = transform(torch.stack(frames, 0))
                return frames, transformFrame
        ...
        print(len(get_next_frame(vid)))
        eval_network(transform_frame(get_next_frame(vid)))

        # rsFrames = cap.read(img_array=True)
        # eval_network(transform_frame(rsFrames[0]))
        print('Done.')
        ...
        if args.display_text or args.display_bboxes:
            ...
            for j in reversed(range(num_dets_to_consider)):
                ...
                if args.display_text:
                    ...
                    cv2.rectangle(img_numpy, (x1, y1), (x1, y1), color, -1)
